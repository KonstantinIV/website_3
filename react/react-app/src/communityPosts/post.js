...
export default class Post extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      starVotedPoints : this.props.post.starVoted ? this.props.post.starVoted : 0,
      starVotes : parseInt(this.props.post.starVotes),
      points    : this.props.post.points === null ? 0 : this.props.post.points 
...
if(type){
 ...
 ajaxApi("/Vote",boolVoted ? "DELETE" : "POST",params, (result,status) => {
console.log(status)
  if(status){
    if(type){
      this.setState({
...
sendStarVote(ID,points){
  //When user has already existing vote
  let a = (parseFloat( this.state.points) * (parseInt(this.state.starVotes))  - this.state.starVotedPoints ) / (parseInt(this.state.starVotes)-1) ; // removin average
  let c = (a === 1/0) ? a :  0; //checking infinite
  let b = ((parseFloat( c) + (parseInt(points) - parseFloat( c)) ) / (parseInt(this.state.starVotes)   )) ; // Adding average


  let params = {ID : ID,
                points : points}
  ajaxApi("/StarVote","POST",params, (result, status) => {
    if(status){
      if(!this.state.starVotedPoints ){ // User Hasnt voted yet
        
        this.setState({ 
          starVotedPoints : parseInt(points),
          starVotes :  parseInt(this.state.starVotes)+1,
          points : parseFloat( this.state.points) + (  (  parseInt(points) - parseFloat( this.state.points) ) / (parseInt(this.state.starVotes)+1 )  )
    ...
    return (
      ...
      <div className={this.state.expandText ? "post_cont expand_text " : "post_cont"} data-id={this.props.post.postID}>
        ...
        <div className="post_user">By
          ...
          <span className="createdDate" date={this.props.post.createdDate} onMouseEnter={() => this.toggleCreatedDate()} onMouseLeave={() => this.toggleCreatedDate()}>
            <div class={this.state.createdDateHover ? "box show" : "box"}> {this.preciseDate(this.props.post.createdDate)}  </div> {this.timeElapsed(this.props.post.createdDate)} 
            </span>
        </div>
      ...
      <div className="post_buttons">
        ...
        : 
          ...
          {!(this.props.post.releaseDate > new Date()) ? 
            ...
            <div class="upvoteWall"> 
                ...
                <a className="commentLinkButton" href={"/comments/"+this.props.post.ID} >
        ...
        <div class="starVoteBar">
          ...
          starVoted={(Math.floor(this.state.points) >= element) ? true : false} 
          starVoteSingle={((Math.floor(this.state.points)+1) === element) ? true : false} 
