...
// 1 cate, 2 vote type, top, search
    ...
    function topPosts($nextCount,$loggedIn, $username,$search,$categoryName,$voteType,$topType,$interval){
     ...
     function newPosts($nextCount,$loggedIn, $username,$search,$categoryName,$voteType,$topType){
    ...
    function getPost($postID,$username){
...
/*
    ...
    //Edit
    function getSPost($username,$postID){
        //echo"asdasd";
        $stmt = $this->pdo->prepare("SELECT 
        post.TOPIC_ID as category, 
        post.title, post.text, 
        DATE_FORMAT(rel_date,'%d/%m/%Y') as releaseDate from post INNER JOIN user ON user.ID = post.USER_ID where username = ? AND post.ID = ?");
